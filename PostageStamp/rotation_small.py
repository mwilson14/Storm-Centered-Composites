...
def calc_regr(A_x, A_y, VEL, xregr_array, yregr_array):
    ...
    for level in [4,12,20,28]:
        for i in range(3, 146, 1):
            for j in range(3, 146, 1):
...
def get_rotation(VEL, REF, rlons_2d, rlats_2d, cenlon, cenlat, bin_size=7):
    ...
    xregr_array = np.zeros((41,150,150))
    yregr_array = np.zeros((41,150,150))
    
    xregr_array, yregr_array = calc_regr(A_x, A_y, VEL, xregr_array, yregr_array)
    
    x_shape = np.arange(0,150,1)

    y_shp, x_shp = np.meshgrid(x_shape, x_shape) 
    print(y_shp)
    # azim = wind_direction(375*units('m/s')-x_shp*units('m/s'), y_shp*units('m/s')-375*units('m/s'))+ 90*units('degree')
    # azim = azim + 270*units('degree')
    lon_c = np.copy(rlons_2d)
    lat_c = np.copy(rlats_2d)
    lon_c[:] = cenlon
    lat_c[:] = cenlat
    print(cenlon, cenlat, 'LLAMA')
    print(np.max(rlons_2d), np.min(rlons_2d))
    print(np.max(rlats_2d), np.min(rlats_2d))
    azim = (g.inv(lon_c, lat_c, rlons_2d, rlats_2d)[0]*units('degree'))+90*units('degree')
    azim[azim<0*units('degree')]=360*units('degree')-azim[azim<0*units('degree')]*-1
    ...
    return az_masked, shear_maxes1, shear_maxes15, shear_maxes2, shear_maxes25, shear_lats1, shear_lats15, shear_lats2, shear_lats25, shear_lons1, shear_lons15, shear_lons2, shear_lons25, azim